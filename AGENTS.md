...
- **Localization from day one.** All user-facing strings go through `IStringLocalizer`. No hardcoded strings in Razor/code. While the i18n scaffold isn't built, **user-facing strings must not contain non-Latin-script letters** — specifically no Cyrillic, Hebrew, Arabic, or Syriac characters. The CI `EnglishOnly` task scans `ts/` and `Pages/` for Unicode blocks `[Ѐ-ԯ]` (Cyrillic+Supplement), `[֐-׿]` (Hebrew), `[؀-ۿ]` (Arabic), `[܀-ݏ]` (Syriac). Basic Latin letters (A–Z a–z), ASCII punctuation, digits, and emoji/symbols like `•` `→` `👁` are fine — use them literally, no HTML-entities needed. Comments in `.cs` under `src/YobaConf.Core` and `tests/` are exempt.
...
- **No inline JavaScript logic in Razor.** All client-side logic lives in `src/YobaConf.Web/ts/` as TypeScript modules, bundled by bun into `wwwroot/js/`. Razor templates only include the bundle via `<script type="module" src="~/js/<name>.js" asp-append-version="true">` in `_Layout.cshtml`. Inline `<script>` blocks containing logic are forbidden — they bypass TypeScript checks, Tailwind JIT scanning, and the lint/format pipeline. External CDN refs (`htmx`, `prismjs`) in `_Layout.cshtml` remain fine — those are not first-party logic. Pattern: new page needs JS → new `ts/<feature>-<what>.ts` + new bundle entry in `package.json` + script tag in `_Layout.cshtml`.
...
- **Formatting:** tool defaults — `dotnet format` for C# (4-space indent), biome default for TS (tab indent, 2 wide). `.editorconfig` at the repo root locks charset / line endings / final newline / trimmed trailing whitespace; nothing overrides the formatter's indent choice. Don't re-add an `indent_style` override — divergence from the tool default correlates with noisier diffs and higher Edit-tool failure rates on smaller coding agents (see `decision-log.md` 2026-04-24).
