...
- **Explicit `include`, no auto ancestor-merge.** Node content is NOT automatically merged with ancestors. Author pulls in what they want via `include "target-ancestor-path"` in `RawContent`. Validation: target must be a proper ancestor of the including node — sibling/descendant/cross-tree includes are parse errors. Fallthrough remains as routing only (missing `/a/b/c` → serve best-match ancestor's content), not as merge. See `decision-log.md` 2026-04-21 "Include поддерживаем, не откладываем".
