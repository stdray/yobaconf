...
Одну из (финально решается в первом hi-fi спринте):

- **`business`** — сине-серая база с тёплым оранжевым accent'ом. Ближе всего к warm-paper вайбу wireframe'а в dark-варианте. Conservative corporate look, подходит под "админка одного юзера".
- **`night`** — холодный deep-blue/black, нейтральный accent. Чище, без "брендового" оттенка.
- **`dark`** — чистый greyscale на чёрном, минимализм. Maximum neutrality.

Моя рекомендация — **`business`**, т.к. warm-accent в тёмной теме читается лучше за длинные админ-сессии, плюс orange ближе к coral-акценту из wireframe'а (единственное, что из визуала wireframe'а приживается без нарушения "no customization").
