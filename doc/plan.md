...
- [ ] **F.1 Stale баннер на главной.** `src/YobaConf.Web/Pages/Index.cshtml:13` — "Storage and resolve engine are being rebuilt. Admin UI lands in Phase B — see doc/plan.md." Phase B сдана, сообщение не актуально. Заменить на краткое описание "что это такое" + ссылки на /Bindings / /admin/users / /admin/api-keys.
...
- [ ] **F.8 "ASCII-only" vs "non-Latin script".** Текущий Cake `EnglishOnly` таск ловит только Cyrillic/Hebrew/Arabic/Syriac (`[Ѐ-ԯ...]`), но AGENTS.md и handoff-брифы говорят "ASCII English only" → pi/opencode эскейпят символы навроде `•` / `👁` через HTML entities / `•` без нужды. Уточнить формулировку в AGENTS.md: "No Cyrillic in user-facing strings; safe characters in Basic Latin / Punctuation / Emoji blocks — allowed literally". Заменить существующие `&#x2022;` / `&#128065;` в `Pages/Bindings/Index.cshtml` на literal `•` / `👁`.
