...
- [ ] **Plain binding value — невалидный JSON-encoded scalar accepted/emitted (баг).** Обнаружено 2026-04-29 при seed'е yobapub bindings: на проде yobaconf жил binding с `keyPath=log.level` и `ValuePlain=information` (bare token, без кавычек), сделавший resolve-вывод `/v1/conf` невалидным JSON: `{"log":{"level":information}}` — consumer-парсер падал на байте 70. Не воспроизведено через `PUT /v1/admin/bindings` с тестовыми данными — нужен repro: либо PUT валидирует/отвергает невалидный JSON-encoded scalar (тогда rogue-row реликт от старого кода), либо resolve должен (re-)quote'ить `Kind=Plain` строки на выходе, либо PUT принимает `value: "raw text"` и serialize'ит дважды правильно. Зафиксировать репро + decision-log entry + фикс. До этого consumer-side скрипты должны парсить ответ defensively (пытаться `JsonDocument.Parse` и логгировать сырое тело — пример в `D:\my\prj\yobapub\scripts\stage1-seed.cs`).
