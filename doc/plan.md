...
- [x] **Plain binding value — невалидный JSON-encoded scalar accepted/emitted (баг).** Обнаружено 2026-04-29 при seed'е yobapub bindings: на проде yobaconf жил binding с `keyPath=log.level` и `ValuePlain=information` (bare token, без кавычек), сделавший resolve-вывод `/v1/conf` невалидным JSON: `{"log":{"level":information}}` — consumer-парсер падал на байте 70. **Fixed 2026-05-01**: два источника — (1) `Edit.cshtml.cs` делал `ValuePlain = ValueText` (сырой пользовательский ввод без JSON-encoding); (2) `ResolvePipeline.DecryptSecrets` не оборачивал не-JSON значения. Fix: Edit page — `JsonDocument.Parse(ValueText).RootElement.GetRawText()` + reject на не-JSON; Resolve — `EnsureJsonScalar()` defence-in-depth. 8 unit-тестов (bare string → quoted, special chars escaped, null/empty → null, already-valid pass-through, numeric/bool pass-through, conflict-diagnostic with bare values).
